...
bool DX_Manager::InitializeDirectX(HWND hWnd)
{
	...
	if (FAILED(hr))
	{
		//	Hardware Vertex Processing may not be supported, so go back to Software Vertex Processing
	...
	// If it not fails, return true
	return true;
}

void DX_Manager::CleanUpDirectX()
{
	for (int i = 0; i < MAX_TEXTURES; i++)
	{
...
void DX_Manager::StartScene()
{
	m_sprite->Begin(D3DXSPRITE_ALPHABLEND);
}

void DX_Manager::EndScene()
{
	m_sprite->End();
...
void DX_Manager::DrawSprite(LPDIRECT3DTEXTURE9 texture, RECT drawRect, D3DXVECTOR3 position)
{
	m_sprite->Draw(texture, &drawRect, NULL, &position, 0xFFFFFFFF);
}

void DX_Manager::DrawSprite(LPDIRECT3DTEXTURE9 texture, D3DXVECTOR3 position)
{
	m_sprite->Draw(texture, NULL, NULL, &position, 0xFFFFFFFF);
}

void DX_Manager::DrawSprite(LPDIRECT3DTEXTURE9 texture)
{
